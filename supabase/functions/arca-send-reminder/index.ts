// Edge Function — mail de rappel chaleureux pour une commande non payée.
// (Portée depuis netlify/functions/send-reminder.js.) ENDPOINT ADMIN.
// POST { order_id } → { success, payment_links } ; exige x-admin-password (fail-closed).
...
Deno.serve(async (req) => {
  ...
  const items = (o.items || []).filter((it: any) => it && it.qty > 0);
  const sousTotal = items.reduce((s: number, it: any) => s + (parseFloat(it.price) || 0) * (parseInt(it.qty) || 0), 0);
  ...
  const totalEur = parseFloat(o.total_eur || 0) || Math.max(0, sousTotal - packDisc - cartDisc + port - shipDisc);
  if (totalEur <= 0) return json(400, { error: "Total à payer = 0 — rien à relancer" });

  const label = `Commande Revue ARCA #${orderId}`;
  let stripeUrl: string | null = null, paypalUrl: string | null = null;
  try { stripeUrl = await createStripePaymentLink(totalEur, label, orderId); }
  catch (e) { console.error("[reminder] stripe link KO:", (e as Error).message); }
  try { paypalUrl = await createPaypalOrder(totalEur, label, orderId); }
  catch (e) { console.error("[reminder] paypal order KO:", (e as Error).message); }

  const prenom = (o.nom || "").trim().split(/\s+/)[0] || "Bonjour";
  const ctx = {
    prenom, nomComplet: o.nom || "", email: o.email, telephone: o.telephone || "",
    adresse: composeAddress(o), pays: o.pays || "", livraison: o.livraison || "",
    mr_code: o.mr_relay_code || "", mr_info: o.mr_relay_info || "", paiement_init: o.paiement || "",
    items, sousTotal, packDisc, cartDisc, shipDisc, port, totalEur, orderId, stripeUrl, paypalUrl,
    iban: "BE92 0017 7210 5023", bic: "GEBABEBB", iban_holder: "Arca Societas",
    communication: "ARCA" + String(orderId).padStart(4, "0"), discount_note: o.discount_note || "",
  };
  const html = buildReminderHtml(ctx);
  const text = buildReminderText(ctx);
  ...
  } catch (_e) { /* ignore si colonne absente */ }

  console.log(`[reminder] envoyé à ${o.email} pour commande #${orderId} (${totalEur} €)`);
  return json(200, { success: true, email: o.email, payment_links: { stripe: stripeUrl, paypal: paypalUrl } });
});

function composeAddress(o: any) {
...
function fmt(n: any) { return Number(n).toFixed(2).replace(".", ",") + " €"; }
function esc(s: any) {
  return String(s == null ? "" : s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}

function buildReminderHtml(d: any) {
  const itemsRows = d.items.map((it: any) => {
  ...
  const totalRow = (lbl: string, val: string, opts: any = {}) => {
...
function buildReminderText(d: any) {
  const lines: string[] = [];
  ...
  lines.push("Antoine de Lophem", "Pour la Revue ARCA · Grez-Doiceau", "");
