// Edge Function — arca-order-submit (ex netlify/functions/submission-created.js).
// KEYSTONE : toutes les voies de commande (form virement, Stripe finalize, PayPal capture)
// convergent ici. Vérifie PayPal côté serveur (anti-forge), envoie 2 mails Brevo (interne +
// client), génère les liens de paiement si non payé, persiste dans arca_orders.
//
// POST { form_name:"commande-arca", data:{…}, _order_id?, _no_persist? }
// Remplace le trigger Netlify Forms : le front (commande.html) POSTera directement ici.
...
// Extrait le code MR à 8 chiffres depuis le barcode 26 chiffres ('41' + code 8 + 16 internes).
function shortMrExp(barcode: any) {
...
Deno.serve(async (req) => {
  try {
    ...
    const fromEmail = arcaEnv("ORDER_EMAIL_FROM") || "";
    ...
    console.log("arca-order-submit invoked, form_name =", formName);

    if (formName !== "commande-arca") {
      return new Response("Ignored (form_name=" + formName + ")", { status: 200 });
    }
    ...
    if (!noPersist && isStripeId && SUPABASE_URL && SUPABASE_KEY) {
      try {
        ...
        if (ck.ok) {
          ...
          if (rows.length > 0) {
            console.log("[Dedup] Commande Stripe déjà traitée (id=" + rows[0].id + "), skip.");
            return new Response("Already processed", { status: 200 });
          }
        }
      } catch (e) {
        console.error("[Dedup] check Supabase échec:", (e as Error).message);
      }
    }
    ...
    if (d.email && !clientEmailValid) {
      console.warn("[Email invalide] '" + d.email + "' — replyTo et envoi client skippés.");
    }
    ...
    if (clientEmailValid) payload.replyTo = { email: d.email, name: d.nom || "" };
    ...
    if (!resp.ok) console.error("Brevo error (interne ARCA):", resp.status, await resp.text());
    ...
    if (!isPaid && orderIdForPay) {
      ...
      if (amountEur > 0) {
        ...
        try { payLinks.stripeUrl = await createStripePaymentLink(amountEur, label, orderIdForPay); }
        catch (e) { console.error("[Stripe link] échec:", (e as Error).message); }
        try { payLinks.paypalUrl = await createPaypalOrder(amountEur, label, orderIdForPay); }
        catch (e) { console.error("[PayPal order] échec:", (e as Error).message); }
      }
    }
    ...
    if (clientEmailValid) {
      try {
        ...
        const clientSubject = isPaid ? `Votre commande ARCA · Paiement reçu` : `Votre commande ARCA · Bien reçue`;
        const clientPayload = {
        ...
        if (!clientResp.ok) console.error("Brevo error (client):", clientResp.status, await clientResp.text());
      } catch (e) {
        console.error("Erreur envoi mail client:", (e as Error).message);
      }
    }
...
// ─────────────────────────────────────────────────────────────
async function verifyPayPalOrder(orderId: string): Promise<any> {
  const CID = arcaEnv("PAYPAL_CLIENT_ID");
  const SEC = arcaEnv("PAYPAL_CLIENT_SECRET");
  if (!CID || !SEC) return { ok: false, reason: "PAYPAL_CLIENT_ID/SECRET non configurés" };
  try {
    const auth = btoa(CID + ":" + SEC);
    ...
    if (!orderResp.ok) {
      ...
      if (orderResp.status === 404) return { ok: false, reason: "Order introuvable côté PayPal (sandbox-vs-live ? autre compte ?)" };
...
// ─────────────────────────────────────────────────────────────
// Persistance dans Supabase (arca_orders)
// ─────────────────────────────────────────────────────────────
async function persistOrder(d: any, mrLabel: any) {
  const { url: SUPABASE_URL, key: SUPABASE_KEY } = supaEnv();
  ...
  const parseN = (m: any) => (m ? parseFloat(m[1].replace(",", ".")) : null);
  ...
  if (!resp.ok) throw new Error(`Supabase POST ${resp.status}: ${(await resp.text()).substring(0, 200)}`);
  console.log("[Supabase] Commande persistée:", d.nom, "/", d.email);
}

// ─────────────────────────────────────────────────────────────
// HTML / texte des emails (inchangé — copié depuis submission-created.js)
// ─────────────────────────────────────────────────────────────
function esc(s: any) {
...
function buildEmailHtml(d: any, mrLabel: any, paypalVerifyWarning: any) {
  const CATALOG: any = {
    1: { title: "N°1", price: 20, badge: null }, 2: { title: "N°2", price: 20, badge: null },
    3: { title: "N°3", price: 20, badge: null }, 4: { title: "N°4", price: 20, badge: null },
    5: { title: "N°5", price: 20, badge: null }, 6: { title: "N°6", price: 20, badge: null },
  ...
  const provider = isStripe ? "Stripe" : isPaypalProvider ? "PayPal" : (d.paiement || "—");
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#fff8ec;border:1px dashed #c8a060;border-radius:4px;">
      <tr><td style="padding:14px 18px;">
        ...
        <table cellpadding="0" cellspacing="0"><tr><td style="background:#c8a060;border-radius:4px;">
          <a href="${esc(adminUrl)}" target="_blank" style="display:inline-block;padding:12px 22px;font:bold 11px Arial;letter-spacing:1.5px;text-transform:uppercase;color:#fff;text-decoration:none;">
...
<table width="640" cellpadding="0" cellspacing="0" border="0" style="max-width:640px;width:100%;background:#fff;border-radius:4px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,.10);">
  ...
  <tr><td style="padding:8px 36px 20px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #e2ddd8;border-radius:4px;overflow:hidden;">
      ...
      <tbody>${qtyRows.join("")}${(Array.isArray(d._custom_items) ? d._custom_items : []).map(function (it: any) {
...
function buildEmailText(d: any, mrLabel: any) {
...
function paymentLabel(paymentMode: any, paypalStatus: any) {
...
function buildClientEmailHtml(d: any, mrLabel: any, payLinks: any) {
  const paypalStatus = d["paypal-status"] || "";
  const isPaid = paypalStatus.startsWith("PAID");
  ...
  const SUB_DEADLINE = new Date("2026-05-25T22:00:00Z");
  const isSubscriptionPeriod = hasN8 && new Date() < SUB_DEADLINE;
  ...
  (Array.isArray(d._custom_items) ? d._custom_items : []).forEach(function (it: any) {
    const sub = (it.qty || 0) * (it.price || 0);
...
function buildClientEmailText(d: any, mrLabel: any, payLinks: any) {
  const paypalStatus = d["paypal-status"] || "";
  const isPaid = paypalStatus.startsWith("PAID");
  const providerLabel = paymentLabel(d.paiement, paypalStatus);
  const isMondialRelay = (d.livraison || "") === "Mondial Relay";
  ...
  if (isPaid) {
    ...
    txt += `  Montant       : ${total} €\n`;
    txt += `  Communication : ${ref}\n\n`;
    if (!isPaypalChoice) txt += `Vous préférez PayPal ?\n  Envoyez ${total} € à : antoine@arca-librairie.com\n\n`;
