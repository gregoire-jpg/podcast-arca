// Helpers e-mail ARCA (Brevo) — partagés par les fonctions d'alerte réassort.
// Réutilise la même config que submission-created.js : BREVO_API_KEY + ORDER_EMAIL_FROM.

const BREVO_URL = "https://api.brevo.com/v3/smtp/email";

// Libellé long / court d'un ouvrage à partir de son numéro (9 = Recueil de prières).
function arcaTitle(num) {
  return Number(num) === 9 ? "le Recueil de prières" : ("la revue ARCA n°" + num);
}
function arcaShort(num) {
  return Number(num) === 9 ? "Recueil de prières" : ("ARCA n°" + num);
}

// Gabarit HTML aux couleurs ARCA (navy + or).
...
module.exports = { arcaTitle, arcaShort, wrapHtml, fromEmail, sendBrevo };
